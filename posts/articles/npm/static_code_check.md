## static code check 静态代码检查

为什么要做静态代码检查？

　　事实上，在开发中，可能会因为个人的疏忽导致代码在不同的浏览器显示不同（不是兼容问题），比如，对象最后一个需不需要逗号的问题，在编译时不会报错，但是在线上环境可能会报错。

　　为了避免出现这样的“低级”错误，这里和大家分享一下javascript的静态代码检查方法。

　　这里使用的是**ESLint**，应用于Gulp中，其他的比如Webpack或者Grunt请自行Google。

首先在项目根目录新建一个.eslintrc文件
在.eslintrc中写入以下内容：
```json
{  
  "env": {  
...
最后，在运行gulp的task时加入lint就可以在consle中打印出相应的检查结果。

最最后，在某些代码我们需要临时改动一下规则，有以下三种方法：
1.禁用ESLint
```javscript
/* eslint-disable */
